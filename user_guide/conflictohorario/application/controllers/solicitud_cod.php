...
class Solicitud extends CI_Controller {
	...
	public function crear($no_validar=''){ //para usuarios no estudiantes la primera vez no debe validar
		...
		if($validacion){			
			...
			$this->enviarCorreo('crear', $datos['sol_email'], 'Creaci�n solicitud', $this->Solicitud_model->insert_id());
	...
	public function actualizar($id){
		...
		if($this->validar()) {
			...
			if((($item[0]['est_id']=='1' || $item[0]['est_id']=='5') && $rol_login=='coordinador') || //en revisi�n o En espera de respuesta del coordinador
	...
	public function comentario($id=''){
		...
		if($id==''){			
			...
			if(!$this->_validar_gestion($item[0]['dep_id'])){			
				...
				if((($item[0]['est_id']=='1'/*En revisi�n*/ || $item[0]['est_id']=='5'/*En espera de respuesta del coordinador*/) && ($this->session->userdata('rol')=='2'/*Coordinador*/ || $this->session->userdata('rol')=='1'/*Administrador*/)) ||
		...
		if($sol_id){
			...
			foreach($lista as $item){
				if(!empty($item)){
					...
					$this->enviarCorreo('cancelar', $datos[0]['sol_email'], 'Cancelaci�n solicitud', $item);					
				}								
			}
		}
		$this->index();
	...
	public function validar(){
		...
		$this->form_validation->set_rules('sol_descripcion', 'Descripci�n', 'max_length[300]');
		$this->form_validation->set_rules('tip_id', 'Tipo', 'required');
		$this->form_validation->set_rules('mov_id', 'Motivo', 'required');
		$this->form_validation->set_rules('sol_tyc', 'Acepta t�rminos y condiciones', 'required');
		
		if($this->input->post('tip_id')=='1') //inscribir materia
			$this->form_validation->set_rules('sol_disp_crn_ret_des', 'CRN Inscripci�n', 'required');
		elseif($this->input->post('tip_id')!=''){
			$this->form_validation->set_rules('sol_disp_crn_ins_des', 'CRN Inscripci�n', 'required');
			$this->form_validation->set_rules('sol_disp_crn_ret_des', 'CRN Retiro', 'required');
		}
		...
		foreach($filas as $item){
			...
			if(is_array($ordencols) && count($ordencols)>0 && $this->session->userdata('ordencol')!=''){
				foreach($ordencols as $ordencol){
					switch ($ordencol) {
						case 'est_id':
							$contenidocelda = "<div title='".$item['est_descripcion']."'>".$color."</div>";
							break;
						case 'tip_id':
			...
			//se deben enviar TODOS los datos en $celda ya que luego se ocultan las columnas
				...
				$celda[] = "<div title='".$item['est_descripcion']."' style='cursor: default'>".$color.( (($item['est_id']=='1')||($item['est_id']=='5')) ? "<img src='http://".$_SERVER['SERVER_NAME']."/conflictohorario/application/core/advertencia.png' style='width: 13px !important; position: absolute; margin-left: -23px;margin-top: 1px; ' />" : "")."</div>";
	...
	private function _get_filas($rol,$inicio,$pr,$order,$sortname,$qtype,$query,$qtype2,$query2){
		
		$filas = array();
		switch($rol){
			case 1:
			 $filas = $this->Solicitud_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query,'','',$qtype2,$query2);		
			break;
			case 2:			 
