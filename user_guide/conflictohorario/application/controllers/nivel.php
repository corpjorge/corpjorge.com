...
class Nivel extends CI_Controller
{
	...
	function index(){
		...
		$data = array('titulo' => 'ADMINISTRADOR DE NIVELES','menu'=>$menu);
		$this->_prepare_list($data);		
		$this->load->view('nivel_listado', $data);
	}
	...
	{	
		...
		if($this->validar()) {
			...
			$mensaje = $this->Nivel_model->insert($data) ? 'Se ha insertado con \u00e9xito' : 'No se pudo insertar';
			$data = array('titulo' => 'ADMINISTRADOR DE NIVELES','mensaje' => $mensaje, 'menu' => $menu);
				$this->_prepare_list($data);
				$this->load->view('nivel_listado', $data);
		}
		else {
			$data = array('titulo'=>'Crear', 'accion'=>'crear', 'niv_id'=>$this->input->post('niv_id'), 'niv_descripcion'=>$this->input->post('niv_descripcion'), 'menu' => $menu);
			$this->load->view('nivel_form', $data);
		}
	}
	...
	{	
		...
		if($this->validar()) {
			$data = array('niv_descripcion' => $this->input->post('descripcion'));
			$mensaje = $this->Nivel_model->update($id, $data) ? 'Se ha actualizado con \u00e9xito' : 'No se pudo actualizar';
			$data = array('titulo' => 'ADMINISTRADOR DE NIVELES','mensaje' => $mensaje, 'menu' => $menu);
			$this->_prepare_list($data);
			$this->load->view('nivel_listado', $data);
		}	
		else {
			$datas = $this->Nivel_model->get_item($id);
	...
	public function borrar($id)
	{
		...
		if($id){
			$mensaje = $this->Nivel_model->delete($id) ? 'Se ha borrado con \u00e9xito' : 'No se pudo borrar';
	...
	private function _prepare_list(&$data){
		$this->load->library('pagination');
                $config_page['base_url'] = '/index.php/nivel/page/';
                $config_page['total_rows'] = $this->Nivel_model->get_count();
                $config_page['per_page'] = 20;		
                $this->pagination->initialize($config_page);
		$data['filas'] = $this->Nivel_model->get_all();
	...
	private function _getmenu(){
        ...
        function page(){
            ...
            $datos['total'] = $this->Nivel_model->get_count();
            $datos['sortname'] = ($this->input->post('sortname')!='')?$this->input->post('sortname'):$this->Nivel_model->tableLlave();
            ...
            foreach($filas as $item){             
                    $datos['rows'][] = array(
                            'id' => $item['niv_id'],				
                            'cell' => array($item['niv_id'], $item['niv_descripcion'])
                    );			
            }
	...
	private function _get_filas($rol,$inicio,$pr,$order,$sortname,$qtype,$query){
		...
		switch($rol){
			case 1:
			 $filas = $this->Nivel_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query);		
			break;
			case 2:			 
			 $programas = explode('*',$this->session->userdata('programas'));
			 $filas = $this->Nivel_model->get_all_coordinador($inicio,$pr,$order,$sortname,$qtype,$query,$programas,'dep_id');			
			break;
			case 3:
			 $login = $this->session->userdata('login');
			 $filas = $this->Nivel_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query,$login,'sol_login');
			break;
		}
		return $filas;
