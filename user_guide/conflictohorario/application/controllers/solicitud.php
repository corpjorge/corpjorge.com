...
//test--------------------------------------------------------------------------------------------------------------------------	
	public function page2(){
		...
		$datos['total'] = $this->_get_count2(($this->input->post('imprimir_consulta')=='1' ? true : false), $this->session->userdata('rol'),$datos['qtype'],$datos['query'],$datos['qtype2'],$datos['query2']);
		
		$inicio = ((int)($datos['page'])-1)*(int)$this->session->userdata('cantpag');		
		
//jmeter-----------------------------------------------------------------------
		$filas = $this->_get_filas2(($this->input->post('rol')!='' ? $this->input->post('rol') : $this->session->userdata('rol')),
							    ($this->input->post('cantpag')!='' ? ((int)($this->input->post('page'))-1)*(int)$this->input->post('cantpag') : $inicio),
								($this->input->post('cantpag')!='' ? $this->input->post('cantpag') : $this->session->userdata('cantpag')),
							    $datos['sortorder'],
							    $datos['sortname'],
                                $datos['qtype'],
                                $datos['query'],
                                $datos['qtype2'],
                                $datos['query2'],
								($this->input->post('imprimir_consulta')=='1' ? true : false));
...
//echo "registros...";print_r($registros);			
	...
	private function _get_filas($rol,$inicio,$pr,$order,$sortname,$qtype,$query,$qtype2,$query2, $imprimir=false){
		...
		switch($rol){
			case 1:
			 $filas = $this->Solicitud_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query,'','',$qtype2,$query2, $imprimir, $fecha_inicial_listado);		
			break;
			case 2:			 
			 $programas = explode('*',$this->session->userdata('programas'));
			 $niveles = explode('*',$this->session->userdata('niveles'));
			 $filas = $this->Solicitud_model->get_all_coordinador($inicio,$pr,$order,$sortname,$qtype,$query,$programas,'dep_id',$programas,$niveles,$qtype2,$query2, $fecha_inicial_listado);		 
			break;
			case 3:
			 $login = $this->session->userdata('login');
			 $filas = $this->Solicitud_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query,'','',$qtype2,$query2, $imprimir, $fecha_inicial_listado);	
			 $filas = $this->Solicitud_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query,$login,'sol_login',$qtype2,$query2, $imprimir, $fecha_inicial_listado);
			break;
		}
		return $filas;
	...
	private function _get_count($rol,$qtype,$query,$qtype2,$query2){
		...
		switch($rol){
			case 1:
			 $filas = $this->Solicitud_model->get_count($qtype,$query,'','',$qtype2,$query2, $fecha_inicial_listado);		
			break;
			case 2:			 
			 $programas = explode('*',$this->session->userdata('programas'));
			 $niveles = explode('*',$this->session->userdata('niveles'));
			 $filas = $this->Solicitud_model->get_count_coordinador($qtype,$query,$programas,'dep_id',$programas,$niveles,$qtype2,$query2, $fecha_inicial_listado);			
			break;
			case 3:
			 $login = $this->session->userdata('login');
			 $filas = $this->Solicitud_model->get_count($qtype,$query,$login,'sol_login',$qtype2,$query2, $fecha_inicial_listado);
			break;
		}
		return $filas;
...
//test--------------------------------------------------------------------------------------------------------------------------
private function _get_filas2($rol,$inicio,$pr,$order,$sortname,$qtype,$query,$qtype2,$query2, $imprimir=false){
		//if($imprimir)echo "<br>rol $rol, inicio $inicio, pr $pr, order $order, sortname $sortname, qtype $qtype, query $query, qtype2 $qtype2, query2 $query2";
		...
		switch($rol){
			case 1:
			 $filas = $this->Solicitud_model_get_filas2->get_all($imprimir, $inicio,$pr,$order,$sortname,$qtype,$query,'','',$qtype2,$query2, $imprimir, $fecha_inicial_listado);		
			break;
			case 2:			 
			 //$programas = explode('*',$this->session->userdata('programas'));
			 //$niveles = explode('*',$this->session->userdata('niveles'));
...
$niveles = explode('*', ($this->input->post('niveles')!='' ? $this->input->post('niveles') : $this->session->userdata('niveles')));
			 $filas = $this->Solicitud_model_get_filas2->get_all_coordinador($imprimir, $inicio,$pr,$order,$sortname,$qtype,$query,$programas,'dep_id',$programas,$niveles,$qtype2,$query2, $fecha_inicial_listado);		 
			break;
			case 3:
			 $login = $this->session->userdata('login');
			 $filas = $this->Solicitud_model_get_filas2->get_all($imprimir, $inicio,$pr,$order,$sortname,$qtype,$query,'','',$qtype2,$query2, $imprimir, $fecha_inicial_listado);	
			 $filas = $this->Solicitud_model_get_filas2->get_all($imprimir, $inicio,$pr,$order,$sortname,$qtype,$query,$login,'sol_login',$qtype2,$query2, $imprimir, $fecha_inicial_listado);
			break;
		}
	...
	private function _get_count2($imprimir, $rol,$qtype,$query,$qtype2,$query2){
		...
		switch($rol){
			case 1:
			 $filas = $this->Solicitud_model_get_count2->get_count($imprimir, $qtype,$query,'','',$qtype2,$query2, $fecha_inicial_listado);		
			break;
			case 2:			 
			 //$programas = explode('*',$this->session->userdata('programas'));
			 //$niveles = explode('*',$this->session->userdata('niveles'));
...
$programas = explode('*', ($this->input->post('programas')!='' ? $this->input->post('programas') : $this->session->userdata('programas')));
$niveles = explode('*', ($this->input->post('niveles')!='' ? $this->input->post('niveles') : $this->session->userdata('niveles')));
			 $filas = $this->Solicitud_model_get_count2->get_count_coordinador($imprimir, $qtype,$query,$programas,'dep_id',$programas,$niveles,$qtype2,$query2, $fecha_inicial_listado);			
			break;
			case 3:
			 $login = $this->session->userdata('login');
			 $filas = $this->Solicitud_model_get_count2->get_count($imprimir, $qtype,$query,$login,'sol_login',$qtype2,$query2, $fecha_inicial_listado);
			break;
		}
...
//------------------------------------------------------------------------------------------------------------------------------
	...
	private function _validar_crear($prog=''){
		...
		if(empty($limites))//no se encontró el programa
			$res = FALSE;
		else
