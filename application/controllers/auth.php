...
class Auth extends CI_Controller {
    ...
    function login($errorMsg = NULL){                
        ...
        if(!$this->auth_ldap->is_authenticated()) {
            ...
            $noticias=  $this->Noticias_model->get_all_published();            
            ...
            if($rules->run() && $this->auth_ldap->login(
                ...
                if($this->session->flashdata('tried_to')) {
                    ...
                    $coordinador = $this->Coordinador_model->ClearFiltros($login,'coo_login');
                    $this->_check_rol($login);                    
                    
                    $file_sess="./session/ci_session_".$this->session->userdata('session_id').".ses";
                    
                    //write_file($file_sess, $this->session->_serialize($this->session->userdata), 'w');
                    
					//echo "hace el redirect --"; exit;
                    //redirect('/solicitud/');
                    
					redirect(base_url().'index.php/solicitud/#test');
                }
            }else {
			
				if(isset($_REQUEST["e"]) && $_REQUEST["e"] == 1){
					$this->load->view('auth/login_form', array('msgExpired'=> 'Su sesión ha caducado por inactividad mayor a 30 min. Por favor ingrese nuevamente','titulo'=>'Login',
                                                           'noticias'=>$noticias
                                                           ));
														   return;
				}
                ...
                $this->load->view('auth/login_form', array('login_fail_msg'=> $errorMsg,'titulo'=>'Login',
                                                           'noticias'=>$noticias
                                                           ));
            }
        }else{
	...
	function login2($errorMsg = NULL){                
        ...
        if(!$this->auth_ldap->is_authenticated()) {
            ...
            if($rules->run() && $this->auth_ldap->login(
				...
				if($rules->set_value('username')!='' && $rules->set_value('password')!='') //logout no muestra $errorMsg
					$errorMsg = 'Usuario o contraseña incorrecta';
                $this->load->view('auth/login_form2', array('login_fail_msg'
...
//-------------------------------------------------------------------------------
    function logout($causa='') {
		//funcionalidad para limpiar los filtros asociados al usuario
		$coordinador = $this->Coordinador_model->ClearFiltros($login,'coo_login');
		//cerrar conexion-------------------------------------------
		global $db_conn;
		if((isset($db_conn) && $db_conn) && !empty($db_conn)){
			oci_close($db_conn);
		}
		//----------------------------------------------------------
    ...
    private function _check_rol($login){
        ...
        if(count($coordinador)>0){
            $dato_sesion = array(
				'rol'=>$coordinador[0]['rol_id'],
				'niveles'=>$coordinador[0]['niv_id'],
				'programas'=>$coordinador[0]['dep_id']/*,
				'materias'=>$coordinador[0]['materia_id'],
				'columnas'=>$coordinador[0]['columnas']*/);
        }else{//estudiante
