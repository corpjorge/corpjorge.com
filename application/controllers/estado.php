...
class Estado extends CI_Controller
{
	...
	public function crear()
	{
		...
		if($this->validar()) {
			$data = array('est_descripcion' => $this->input->post('descripcion'),'est_padre'=>$this->input->post('est_padre'));
			...
			$data = array('titulo'=>'Crear', 'accion'=>'crear', 'est_id'=>$this->input->post('est_id'), 'est_descripcion'=>$this->input->post('est_descripcion'), 'menu' => $menu,'options_estado'=>$options_estado);
	...
	{	
		...
		if($this->validar()) {
			...
			$accion = array('titulo'=>'Actualizar', 'accion'=>'actualizar', 'menu' => $menu,'options_estado'=>$options_estado);
	...
	private function _getmenu(){
        ...
        function page(){
            ...
            foreach($filas as $item){             
                    $padre=$this->Estado_model->get_item($item['est_padre']);
                    
                    $item['est_padre']=(count($padre)>0)?$padre[0]['est_descripcion']:"";
                    $datos['rows'][] = array(
                            'id' => $item['est_id'],				
                            'cell' => array($item['est_id'], $item['est_descripcion'], $item['est_padre'])
                    );			
            }
