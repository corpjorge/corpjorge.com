...
class Reporte extends CI_Controller
{
	...
	function index(){
		...
		//Se consultan todos los periodos de las sol creadas.
		$data['option_perid'] = array();
		$data['option_perid'] = $this->Estado_model->get_dropdown_peridos(TRUE);
	...
	public function busquedaReporte(){
		...
		$periodo_id = ($this->input->post('periodo_id'))?$this->input->post('periodo_id'):' ';
	...
	public function page(){
		...
		$datos['otro'] =($this->input->post('otro')!='n')?$this->input->post('otro'):'';
		if(!$datos['otro'] &&  $this->session->userdata('otrodg') && $this->session->userdata('otrodg') != 'n')
			$datos['otro'] = $this->session->userdata('otrodg');
			
		//echo $datos['otro']; exit;
		...
		$periodo_id = @$otrosparams[5];
		$fecha_ini = @$otrosparams[2];
		$fecha_fin = @$otrosparams[3];
		$reporte = @$otrosparams[4];
		
		if($datos['otro']){
			$datos_sesion = array('otrodg'=>$datos['otro']);
			$this->session->set_userdata($datos_sesion);
			//print_r($this->session->userdata('otrodg'));
		}
		...
		switch($reporte){			
			case '1':				
				$filas = $this->Solicitud_model->reporte_solicitudes(TRUE,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);				
				$datos['total'] = $this->Solicitud_model->count_reporte_solicitudes($programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);
				foreach($filas as $item){
					...
					$estado = $this->Estado_model->get_item($item['est_id']);
					$estadoPadre = $this->Estado_model->get_item($estado[0]['est_padre']);
                                        $est=$estado[0]['est_descripcion'];
                                        $estPadre='';                                        
					...
					$datos['rows'][] = array(
						...
						'cell' => array($item['sol_id'],
								...
								$estPadre,						
								$est,
								$item['sol_opcion_estud'],
								$item['sol_ssc'],
								$item['sol_fec_est_actualiza'])
					);		
				}				
			break;
			case '2':
				$filas = $this->Solicitud_model->reporte_solicitud_estado(true,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);
				$datos['total'] = $this->Solicitud_model->count_reporte_solicitud_estado($programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);		
				foreach($filas as $item){
					$id++;
					$datos['rows'][] = array(
						'id' => $id,				
			...
			break;
			case '3':
				$filas = $this->Solicitud_model->reporte_solicitud_crn(true,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);
				$datos['total'] = $this->Solicitud_model->count_reporte_solicitud_crn($programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);		
				foreach($filas as $item){
					$id++;
					$datos['rows'][] = array(
						'id' => $id,				
			...
			break;
			case '4':
				$filas = $this->Solicitud_model->reporte_solicitud_basico(true,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);
				//$datos['total'] = count($filas);
				$datos['total'] = $this->Solicitud_model->count_reporte_solicitud_basico($programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, $inicio, $this->input->post('rp'), $datos['sortname'], $datos['sortorder'],$mis_prog,$mis_niveles);		
                                $this->load->model('Comentario_model','',TRUE);
				
				
				foreach($filas as $item){
                                        $comentario = $this->Comentario_model->get_item($item['sol_id'],'sol_id');
                                        $datosComentario='';
                                        foreach ($comentario as $value) {
                                            $datosComentario.= " Fecha: ".$value['com_fecha']." ".$value['com_nombre']." dijo: ". strip_tags($value['com_texto']) ." -- ";
                                        }
					$id++;
					$datos['rows'][] = array(
						'id' => $id,				
	...
	public function excel(){
		...
		$periodo_id = ($this->input->post('periodo_id_2'))?$this->input->post('periodo_id_2'):'';
		...
		switch($reporte){
			case '1':
				...
				$filas = $this->Solicitud_model->reporte_solicitudes(false,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, 0, 50, 'sol_id', 'ASC',$mis_prog,$mis_niveles);
				$cabeceras = array('ID','Cod Materia','CRN','Materia','Seccion','Profesor','Descripcion','Tipo','Motivo','Fecha','Login','Estudiante','Apellido','Programa','Doble Programa','Creditos Acumulados','Periodo','Codigo','Estado Principal','Sub Estado','Opcion Estudiante','SSC','Primer Semestre','Fecha ultimo estado');

		if ($_SERVER['HTTP_CLIENT_IP'] == '157.253.93.36') {
#			echo count ($filas);
#			echo "<pre>";
#			print_r ($filas);
#			exit;
			...
			case '2':
				...
				$filas = $this->Solicitud_model->reporte_solicitud_estado(false,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, 0, 50, 'dep_id', 'ASC',$mis_prog,$mis_niveles);
			...
			case '3':
				...
				$filas = $this->Solicitud_model->reporte_solicitud_crn(false,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, 0, 50, 'sol_ins_crn', 'ASC',$mis_prog,$mis_niveles);
			...
			break;
                        case '4':
                                $nombre = 'solicitudes_'.time();
				$cabeceras = array('ID','Codigo','Nombre','Apellido','CRN','Periodo','Materia','Descripcion','Comentario 1','Comentario 2','Comentario 3','Comentario 4','Comentario 5','Comentario 6');
				$filas = $this->Solicitud_model->reporte_solicitud_basico(false,$programa_id, $estado_id, $periodo_id, $fecha_ini, $fecha_fin, 0, 50, 'sol_id', 'ASC',$mis_prog,$mis_niveles);	
				...
				foreach($filas as $item){
                                    ...
                                    //$datosComentario=  implode("  -  ", $datosComentario);
                                    
                                    $citem=array();
                                    for($i=0; $i<=6 ; $i++){
                                    	if(isset($datosComentario[$i]))$citem['comentarios'.$i]=$datosComentario[$i];
                                    	else $citem['comentarios'.$i]="";
                                    }
                                    $datos[] = array($item['sol_id'],
                                                        $item['sol_uidnumber'],
                                                        $item['sol_nombre'],
