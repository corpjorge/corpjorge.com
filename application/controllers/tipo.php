...
class Tipo extends CI_Controller
{
	...
	function index(){
		...
		$data = array('titulo' => 'ADMINISTRADOR DE TIPOS','menu'=>$menu);
		$this->_prepare_list($data);		
		$this->load->view('tipo_listado', $data);
	}
	...
	{	
		$menu = $this->Menu_model->_getmenu();
		if($this->validar()) {
			$data = array('tip_descripcion' => $this->input->post('descripcion'));
			$mensaje = $this->Tipo_model->insert($data) ? 'Se ha insertado con \u00e9xito' : 'No se pudo insertar';
			$data = array('titulo' => 'ADMINISTRADOR DE TIPOS','mensaje' => $mensaje, 'menu' => $menu);
				$this->_prepare_list($data);
				$this->load->view('tipo_listado', $data);
		}
		else {
			$data = array('titulo'=>'Crear', 'accion'=>'crear', 'tip_id'=>$this->input->post('tip_id'), 'tip_descripcion'=>$this->input->post('tip_descripcion'), 'menu' => $menu);
			$this->load->view('tipo_form', $data);
		}
	}
	...
	{	
		...
		if($this->validar()) {
			$data = array('tip_descripcion' => $this->input->post('descripcion'));
			$mensaje = $this->Tipo_model->update($id, $data) ? 'Se ha actualizado con \u00e9xito' : 'No se pudo actualizar';
			$data = array('titulo' => 'ADMINISTRADOR DE TIPOS','mensaje' => $mensaje, 'menu' => $menu);
			$this->_prepare_list($data);
			$this->load->view('tipo_listado', $data);
		}	
		else {
			$datas = $this->Tipo_model->get_item($id);
	...
	public function borrar($id)
	{
		...
		if($id){
			$mensaje = $this->Tipo_model->delete($id) ? 'Se ha borrado con \u00e9xito' : 'No se pudo borrar';
	...
	private function _prepare_list(&$data){
		$this->load->library('pagination');
                $config_page['base_url'] = '/index.php/tipo/page/';
                $config_page['total_rows'] = $this->Tipo_model->get_count();
                $config_page['per_page'] = 20;		
                $this->pagination->initialize($config_page);
		$data['filas'] = $this->Tipo_model->get_all();
	...
	private function _getmenu(){
        ...
        function page(){
            ...
            $datos['total'] = $this->Tipo_model->get_count();
            $datos['sortname'] = ($this->input->post('sortname')!='')?$this->input->post('sortname'):$this->Tipo_model->tableLlave();
            ...
            foreach($filas as $item){             
                    $datos['rows'][] = array(
                            'id' => $item['tip_id'],				
                            'cell' => array($item['tip_id'], $item['tip_descripcion'])
                    );			
            }
	...
	private function _get_filas($rol,$inicio,$pr,$order,$sortname,$qtype,$query){
		...
		switch($rol){
			case 1:
			 $filas = $this->Tipo_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query);		
			break;
			case 2:			 
			 $programas = explode('*',$this->session->userdata('programas'));
			 $filas = $this->Tipo_model->get_all_coordinador($inicio,$pr,$order,$sortname,$qtype,$query,$programas,'dep_id');			
			break;
			case 3:
			 $login = $this->session->userdata('login');
			 $filas = $this->Tipo_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query,$login,'sol_login');
			break;
		}
		return $filas;
