...
//$prog = 'PRUEBA'; ////////////////////////////////////////////prueba eliminar
			...
			if(count($validaCrn)>0){
				...
				$msj='Su solicitud de conflicto de Horario se ha enviado con \u00e9xito';
			}
...
//$item[0]['dep_id'] = 'PRUEBA'; ////////////////////////////////////////////prueba eliminar
			$hiddeActions=0;
			if(!$this->_validar_gestion($item[0]['dep_id'])){$hiddeActions=1;}
...
//			}else{
		...
		if($sol_id){
			...
			foreach($lista as $item){
				if(!empty($item)){
					...
					$this->enviarCorreo('cancelar', $datos[0]['sol_email'], 'Cancelaciï¿½n solicitud', $item);
				}
			}
		}
		$this->index();
	...
	public function estado(){
		...
		//$this->index();
		if($this->input->post('sol_id_siguiente'))$this->formaestado($this->input->post('sol_id_siguiente'));
		else $this->index();
	}
	...
	public function page(){
		...
		//si viene algo por sesion en qtype lo pasa al post y lo elimina de la sesion
			$limpiar = false;
			// if( trim($this->input->post('query')) == ''
				// && trim($this->input->post('query2')) == ""
				// && !$this->input->post('query')
				// && ($this->input->post('query3') == "0" || !$this->input->post('query3') || $this->input->post('query3') == null || $this->input->post('query3') == 'null'))
					// $limpiar = true;
		...
		$query3 = ($this->input->post('query3')
					&& $this->input->post('query3') != null
					&& $this->input->post('query3') != 'null') ? $this->input->post('query3') : $this->session->userdata('query3') ;
		// var_dump($query3);
		// var_dump($limpiar);
		//echo $this->input->post('query3'); exit;
		$datos_sesion= array(
			'qtype'=>$qtype,
			'query'=>$query,
			'qtype2'=>$qtype2,
		...
		$datos['total'] = $this->_get_count($this->session->userdata('rol'),$datos['qtype'],$datos['query'],$datos['qtype2'],$datos['query2'],$datos['query3']);
			...
			//echo ($datos['page']-1) . " => ".((int)$datos['total']/(int)$this->session->userdata('cantpag')); exit;
...
//			$periodo = $periodo[0]['par_valor'];
//			//
//			$datos_estudiante = $this->integracion->datosEstudiante($item['sol_pidm'],$periodo);
//
//			$item['prog_doble'] = $datos_estudiante['DOBLE_PROGRAMA'];
			...
			$img=(AMBIENTE_PRUEBAS=='1')?$_SERVER['SERVER_NAME']."/conflictohorario/application/core/advertencia.png":$_SERVER['SERVER_NAME']."/application/core/advertencia.png";

			$celda[] = $item['sol_id'];

			$celda[] = "<div title='".$item['est_descripcion']."' style='position: relative; cursor: default'>".$color.( ((($item['est_id']=='1')) && $this->session->userdata('rol')!=3) ? "<img src='http://".$img."' style='width: 13px !important; position: absolute; margin-left: -23px;margin-top: 1px; ' />" : "").($item['cta']>0 && $rolI!=3 ? "<div class='alert_msg_com' title='Comentarios sin leer'>".($item['cta']>99 ? "+".$item['cta'] : $item['cta'])."</div>" : "")."</div>";
...
//------------------------------------------------------------------------------------------------------------------------------
	...
	public function ayudaMinicartelera($tip_id, $id_input, $valor='', $tipo='', $pidm='' , $codEstudiante=0, $periodo){
		...
		//$data['id_input'] = $id_input;
		$data['valor'] = $valor;
		$data['idEstudiante'] = $codEstudiante;
		$data['tipo'] = $tipo;
		$data['pidm'] = $pidm;
...
$aviso = empty($limites_dep) ? 'No se encuentra registrado el programa con ID: '.$item[0]['dep_id'] : 'El Periodo de gesti&oacute;n de solicitudes ha finalizado.';
			...
			if($contador == count($lista)){
				...
				$ordenadas = $this->_get_filas($this->session->userdata('rol'),
									...
									0,$this->session->userdata('cantpag'),
									$filtros['sortorder'],
									$filtros['sortname'],
									$filtros['qtype'],
									$filtros['query'],
	...
	public function mensaje($tipo, $dep_id=''){
		...
		switch($tipo){
			case 1:
				...
				break;
			case 4:
				$mensaje = 'La creaci&oacute;n de solicitudes para ['.$dep_id.'] se encuentra desactivado.';
				break;
			case 2:
				break;
			case 3:
...
$aviso = empty($limites_dep) ? 'No se encuentra registrado el programa con ID: '.$item[0]['dep_id'] : 'El Periodo de gesti&oacute;n de solicitudes ha finalizado.';
	...
	public function comentar_masivo(){
        ...
        public function validarCrnEstudiante(){
            ...
            $this->load->model('Parametro_model','',TRUE);
            $resultado=$this->Solicitud_model->validarCrnBloqueado($crn);


            if(count($resultado)>0){
                $rta['msj']=  utf8_encode("La creación de solicitudes para el CRN ". ($crn)." ha sido bloqueada");
                $rta['pass']=0;
            }  else {
                $this->load->model('Parametro_model','',TRUE);
                // $periodo = $this->Parametro_model->get_item('periodo','par_nombre');
	...
	public function horario($id){
		...
		echo $this->integracion->horario_estudiantes($pidm, $periodo, $d[0]["sol_nombre"], $d[0]["sol_apellido"]);
		exit;
	}
}