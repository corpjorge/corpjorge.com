...
class Departamento extends CI_Controller {
	...
	function actualizarProgramas(){
		...
		$programas = $this->integracion->obtenerProgramas($periodo);
		$this->Departamento_model->cruzarProgramas($programas);
	...
	public function actualizar($id=''){
		...
		if(/*$this->validar()*/$id == '') {
			...
			$data_llmite = array('dep_id'=>$id,
					     ...
					     'sol_creacion'=>@$limite[0]['sol_creacion']
					     );
	...
	public function actSol($d){
		$d = "'".implode("','", explode("-", $d))."'";
		$this->Departamento_model->actSol($d);
		$menu = $this->Menu_model->_getmenu();
		$data = array('titulo' => 'ADMINISTRADOR DE PROGRAMAS','mensaje'=>'Se ha actualizado con \u00e9xito','menu'=>$menu);
		$this->_prepare_list($data);
		$this->load->view('departamento_listado', $data);
	...
	public function desSol($d){
		$d = "'".implode("','", explode("-", $d))."'";
		$this->Departamento_model->desSol($d);
		$menu = $this->Menu_model->_getmenu();
		$data = array('titulo' => 'ADMINISTRADOR DE PROGRAMAS','mensaje'=>'Se ha actualizado con \u00e9xito','menu'=>$menu);
		$this->_prepare_list($data);
		$this->load->view('departamento_listado', $data);
	...
	public function relate($dep_id){
		...
		$data = array(
			...
			'sol_creacion'=>$this->input->post('sol_creacion'),
			'dep_id'=>$dep_id,
	...
	public function page(){
		...
		// print_r($filas);
		// echo "</pre>";
		$_limites = $this->Departamento_model->get_limits();
		$limites = array();
		foreach($_limites as $l){
			$limites[$l["dep_id"]] = $l["sol_creacion"];
		}
		...
		foreach($filas as $item){
			...
			//\u00d3n
			$sol_creacion = (int)@$limites[$item['dep_id']]==1 ? "Activado" : "Desactivado";
			$datos['rows'][] = array(
				'id' => $item['dep_id'],
				'cell' => array($item['dep_id'], /*$item['dep_nombre']*/$dep_nombre, $sol_creacion/*, $item['dep_externo']*/)
			);
		}
		//print_r($datos);
	...
	private function _prepare_list(&$data){
		...
		foreach($filas as $item){			
			$datos['rows'][] = array(
				'id' => $item['dep_id'],
				'cell' => array($item['dep_id'], $item['dep_nombre'], $item['sol_creacion'], $item['dep_externo'])
			);
		}
