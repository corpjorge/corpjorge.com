...
class Coordinador extends CI_Controller
{
	...
	public function gOrden(){
		$this->Coordinador_model->updOrderColumn();
		exit;
	}
	
	public function cColumnas(){
		echo $this->Coordinador_model->cColumnas();
		exit;
	}
	
	public function cPermisos(){
		echo $this->Coordinador_model->cargarPermisos();
		exit;
	}
	...
	{	$menu = $this->Menu_model->_getmenu();
		...
		if($this->validar()) {
			$r = $this->db->query("SELECT materia_id, dep_id, materia_id2, dep_id2 FROM ch_coordinador WHERE coo_id='".$id."' LIMIT 1");
			$r = $r->result(); 
			$materia_id2 = $r[0]->materia_id;
			$materias_ids2 = explode("*", $materia_id2);
			$materias_ids1 = $this->input->post('materias_ids');
			$materias_dif1 = (boolean)count(array_diff($materias_ids1, $materias_ids2));
			$materias_dif2 = (boolean)count(array_diff($materias_ids2, $materias_ids1));
			
			$dep_id2 = $r[0]->dep_id;
			$dep_ids2 = explode("*", $dep_id2);
			$dep_ids1 = $this->input->post('dep_ids');

			$dep_dif1 = (boolean)count(array_diff($dep_ids1, $dep_ids2));
			$dep_dif2 = (boolean)count(array_diff($dep_ids2, $dep_ids1));

			$dep_id = '';
			if(is_array($this->input->post('dep_ids'))) {
				$dep_id = implode("*", $this->input->post('dep_ids'));
			}
			else
				$dep_id = $this->input->post('dep_ids');
                        ...
                        $materia_id = '';
			if(is_array($this->input->post('materias_ids'))) {
				$materia_id = implode("*", $this->input->post('materias_ids'));
			}
			else
				$materia_id = $this->input->post('materias_ids');
			...
			'dep_id'=>$dep_id,
			'materia_id'=>$materia_id,
			'dep_id2'=>($dep_dif1 || $dep_dif2 ? $dep_id2 : ($r[0]->dep_id2=="" ? $dep_id : $r[0]->dep_id2)),
			'materia_id2'=>($materias_dif1 || $materias_dif2 ? $materia_id2 : ($r[0]->materia_id2=="" ? $materia_id : $r[0]->materia_id2)),
			'rol_id'=>$this->input->post('rol_id'),
			);
			// echo "<pre>";
			// print_r($data);
	...
	public function recargar_permisos(){
		$datos = $this->input->post("datos");
		$d = array();
		foreach($datos as $a){
			$t = array();
			$sel = $this->Coordinador_model->get_selMats($a);
			if(trim(@$sel[0]["dep_id"])!=""){
				$sels = explode("*",@$sel[0]["dep_id"]);
				$_m = array();
				foreach($sels as $s){
					$t[] = $s;
					$materias = $this->integracion->materiasPorDepto($s); 
					foreach($materias as $m){
						$_m[] = $m["MATERIA"]."".$m["CURSO"];
					}
				}
				$d[$a]["dep_id"] = implode("*", $t);
				$d[$a]["materia_id"] = implode("*", $_m);
			}
		}
		foreach($d as $k=>$i){
			$this->Coordinador_model->actualizarPermisos($k, $i["dep_id"], $i["materia_id"]);
		}
		exit;
	}
	
        public function getMaterias($id,$coo_id, $ant=""){
            //echo "<li>" .$id =  $this->input->post('id');            
            $materiasSeleccionadas=array();
            if($coo_id>0){
                $materiasSeleccionadas = $this->Coordinador_model->get_selMats($coo_id);
                $materiasSeleccionadas = $ant=="" ? $materiasSeleccionadas[0]['materia_id'] : $materiasSeleccionadas[0]['materia_id2'];
