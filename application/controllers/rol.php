...
class Rol extends CI_Controller
{
	...
	function index(){
		...
		$data = array('titulo' => 'ADMINISTRADOR DE ROLES','menu'=>$menu);		
		$this->_prepare_list($data);		
		$this->load->view('rol_listado', $data);
	}
	
	public function crear()	{	
		$menu = $this->Menu_model->_getmenu();
		if($this->validar()) {
			$data = array('rol_descripcion' => $this->input->post('descripcion'));
			$mensaje = $this->Rol_model->insert($data) ? 'Se ha insertado con \u00e9xito' : 'No se pudo insertar';
			$data = array('titulo' => 'ADMINISTRADOR DE ROLES','mensaje' => $mensaje, 'menu' => $menu);
				$this->_prepare_list($data);
				$this->load->view('rol_listado', $data);
		}
		else {
			$data = array('titulo'=>'Crear', 'accion'=>'crear', 'rol_id'=>$this->input->post('rol_id'), 'rol_descripcion'=>$this->input->post('rol_descripcion'), 'menu' => $menu);
			$this->load->view('rol_form', $data);
		}
	}
	...
	{	
		...
		if($this->validar()) {
			$data = array('rol_descripcion' => $this->input->post('descripcion'));
			$mensaje = $this->Rol_model->update($id, $data) ? 'Se ha actualizado con \u00e9xito' : 'No se pudo actualizar';
			$data = array('titulo' => 'ADMINISTRADOR DE ROLES','mensaje' => $mensaje, 'menu' => $menu);
			$this->_prepare_list($data);
			$this->load->view('rol_listado', $data);
		}	
		else {
			$datas = $this->Rol_model->get_item($id);
	...
	public function borrar($id)
	{
		...
		if($id){
			$mensaje = $this->Rol_model->delete($id) ? 'Se ha borrado con \u00e9xito' : 'No se pudo borrar';
	...
	private function _prepare_list(&$data){
		$this->load->library('pagination');
                $config_page['base_url'] = '/index.php/rol/page/';
                $config_page['total_rows'] = $this->Rol_model->get_count();
                $config_page['per_page'] = 20;		
                $this->pagination->initialize($config_page);
		$data['filas'] = $this->Rol_model->get_all();
	...
	function search()
	{
        ...
        function page(){
            ...
            $datos['total'] = $this->Rol_model->get_count();
            $datos['sortname'] = ($this->input->post('sortname')!='')?$this->input->post('sortname'):$this->Rol_model->tableLlave();
            ...
            foreach($filas as $item){             
                    $datos['rows'][] = array(
                            'id' => $item['rol_id'],				
                            'cell' => array($item['rol_id'], $item['rol_descripcion'])
                    );			
            }
	...
	private function _get_filas($rol,$inicio,$pr,$order,$sortname,$qtype,$query){
		...
		switch($rol){
			case 1:
			 $filas = $this->Rol_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query);		
			break;
			case 2:			 
			 $programas = explode('*',$this->session->userdata('programas'));
			 $filas = $this->Rol_model->get_all_coordinador($inicio,$pr,$order,$sortname,$qtype,$query,$programas,'dep_id');			
			break;
			case 3:
			 $login = $this->session->userdata('login');
			 $filas = $this->Rol_model->get_all($inicio,$pr,$order,$sortname,$qtype,$query,$login,'sol_login');
			break;
		}
		return $filas;
