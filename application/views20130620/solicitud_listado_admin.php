...
$(document).ready(function() {    
    ...
    $("#flex1").flexigrid({
      ...
      colModel : [	
		...
		$columnas22 = array();		
		$ordencol22 = $ordencol;
		$ordencols = explode(';', trim($ordencol));
		$ordencol_label = explode(';', trim($ordencol_label));
		$ordencol_width = explode(';', trim($ordencol_width));		
		if(is_array($ordencols) && count($ordencols)>0 && $ordencol!=''){
			foreach($ordencols as $indice=>$ordencol){
				...
				$columnas22[] = "{display: '".htmlentities(utf8_decode($ordencol_label[$indice]))."', name : '".$ordencol."', width : ".str_replace('px', '', $ordencol_width[$indice]).", sortable : true, align: 'left', hide: ".(in_array($ordencol,$ocultas) ? 'true' : 'false')."} $indice - ".(count($ordencols) - 2).($indice<(count($ordencols) - 2) ? ',': '');
				?>
					{display: '<?php echo htmlentities(utf8_decode($ordencol_label[$indice])); ?>', name : '<?php echo $ordencol; ?>', width : <?php echo str_replace('px', '', $ordencol_width[$indice]); ?>, sortable : true, align: 'left', hide: <?php $hide = in_array($ordencol,$ocultas) ? 'true' : 'false'; echo $hide; ?>}<?php if($indice<(count($ordencols) - 2))echo ',' ?>
				<?php }
			}
		}
		else {	?>		
			<?php //if(!in_array('sol_id',$ocultas)){?>
			{display: 'ID', name : 'sol_id', width : /*40*/40, sortable : true, align: 'left', hide: <?php $hide = in_array('sol_id',$ocultas) ? 'true' : 'false'; echo $hide; ?>},
			<?php //}?>
			...
			{display: 'Programa', name : 'sol_prog', width : /*50*/160, sortable : true, align: 'left', hide: <?php $hide = in_array('sol_prog',$ocultas) ? 'true' : 'false'; echo $hide; ?>},
			<?php //if(!in_array('sol_ins_mat',$ocultas)){?>
			{display: 'Doble Programa', name : 'sol_doble_prog', width : /*50*/160, sortable : true, align: 'left', hide: <?php $hide = in_array('sol_doble_prog',$ocultas) ? 'true' : 'false'; echo $hide; ?>},
			<?php //if(!in_array('sol_ins_mat',$ocultas)){?>
			{display: 'Cr&eacute;ditos Acumulados', name : 'sol_creditos', width : /*50*/100, sortable : true, align: 'left', hide: <?php $hide = in_array('sol_creditos',$ocultas) ? 'true' : 'false'; echo $hide; ?>},
			<?php //}?>
			<?php //if(!in_array('sol_ins_mat',$ocultas)){?>
			{display: 'Periodo', name : 'sol_periodo', width : /*50*/60, sortable : true, align: 'left', hide: <?php $hide = in_array('sol_periodo',$ocultas) ? 'true' : 'false'; echo $hide; ?>},
			<?php //}?>
			<?php //if(!in_array('sol_ins_mat',$ocultas)){?>
...
<?php
/*echo "$ordencol22<br>";
foreach($columnas22 as $columna22){
	echo $columna22.'<br>';
}*/
