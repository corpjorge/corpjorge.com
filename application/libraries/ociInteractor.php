	...
	function __construct(){    
		...
		$this->port =DB_PORT;
				...
				if(!(isset($db_conn) && $db_conn)){
					$db_conn = ocilogon ($this->login, $this->password, '//'.$this->server.':'.$this->port.'/'.$this->database)or die( "Could not connect to Oracle database!"); 
					/*echo '<!--<pre>';
					print_r($this->server);
					print_r($this->login);
					print_r($this->port);
		...
		if ($this->esta_conectado){
			$resultado = oci_parse($this->db_conn, utf8_decode($query));
			if ($resultado){
