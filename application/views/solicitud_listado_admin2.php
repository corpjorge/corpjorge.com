		...
		if(is_array($ordencols) && count($ordencols)>0 && $ordencol!=''){
			foreach($ordencols as $indice=>$ordencol){
				if(/*!in_array($ordencol, $ocultas) && */$ordencol!=''){ ?>
					{display: '<?php echo htmlentities(utf8_decode($ordencol_label[$indice])); ?>', name : '<?php echo $ordencol; ?>', width : <?php echo str_replace('px', '', $ordencol_width[$indice]); ?>, sortable : true, align: 'left', hide: <?php $hide = in_array($ordencol,$ocultas) ? 'true' : 'false'; echo $hide; ?>},
				<?php }
			}
		}
		else {	?>		
