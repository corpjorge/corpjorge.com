...
$(document).ready(function(){
       ...
       $('#crnval').autocomplete({   
            ...
            // source :'autocomplete?periodo=' + jQuery("#periodo").val(),
            source: function(request, response) {
                $.getJSON("autocomplete/", { 
                    periodo: jQuery("#periodo").val(),
                    term: jQuery("#crnval").val()
                }, response);
            },
...
$dperiodo	=	explode(":=:",trim($dperiodo));
...
<?php echo form_open('bloqueo/'.$accion.'/'.$blq_id); ?>
	...
	<p>
		<label for="crn">Período: </label>
			<select name="periodo" id="periodo">
				<?php
				if(count($periodo)>0){
					foreach($periodo as $k=>$p){
						echo "<option value='$p'>$p - ".$dperiodo[$k]."</option>";
					}
				}else{
