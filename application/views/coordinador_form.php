...
function materias(id,coo_id,ant){  
		// console.log("#mat"+id)
    if($('#dep_ids_'+id).is(':checked')){
        if(!$("#mat"+id).length){            
            ...
            $.ajax({
                url: '<?php echo base_url()?>index.php/coordinador/getMaterias/'+id+'/'+coo_id+'/'+ant,
                //data: 'id='+id,
                type: "POST",
                dataType:"json",
...
$(document).ready(function(){
	...
	jQuery("#btn_recargar").click(function(){
		// Al pasar a produccion quitar "/conflictohorario" 
		jQuery.ajax({
			url: "/conflictohorario/index.php/coordinador/cPermisos", 
			type: 'post',
			dataType: 'json',
			data: {
				login : "<?php echo $coo_login; ?>"
			},
			success: function(d){
				jQuery(".programasLst input[type='checkbox']").removeAttr("checked");
				// jQuery(".programasLst input[type='checkbox']:checked").click();
				for(var i=0;i<d.dep_id2.length;i++){
					// jQuery(".programasLst input[value='"+d.dep_id2[i]+"']").attr("checked","checked");
					jQuery(".programasLst input[value='"+d.dep_id2[i]+"']").click();
					// alert(d.dep_id2[i])
					materias(d.dep_id2[i], "<?php echo $coo_id; ?>", "si");
					// jQuery(".programasLst input[value='"+d.dep_id2[i]+"']").parent().parent().parent().find("+ tr input[type='checkbox']").removeAttr("checked");
				}
				// for(var i=0;i<d.materia_id2.length;i++){
					// jQuery(".programasLst input[value='"+d.materia_id2[i]+"']").attr("checked","checked");
				// }
			}
		});
	});
...
<?php echo form_open('coordinador/'.$accion.'/'.$coo_id); ?>
	...
	<tr>
		<table class='formtable programasLst' width="80%" style="border:1px #F2F2F2 solid" border="0">
			<tr>
				<td class="cellh"><strong>Programa</strong></td>
				<!--<td class="cellh"><strong>Id</strong> <img src="order2.gif" border="0" /></td>-->
				<td class="cellh" align="right"> <!--<img src="order2.gif" border="0" />-->
					<?php if($coo_id){ ?>
					<!--input type="button" id="btn_recargar" style="width: auto" value="Recargar permisos" /-->
					<?php } ?>
				</td>
			</tr>
