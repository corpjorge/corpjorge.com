...
class Solicitud_model extends CI_Model 
{
	...
	*/
	...
	public function get_count($qtype='',$qcampo='',$where='',$campo_where='',$qtype2='',$qcampo2='', $fecha_inicial_listado=''){
	...
	public function get_count_coordinador($qtype='',$qcampo='',$where=array(),$campo_where='',$arr_prog,$arr_nivel,$qtype2='',$qcampo2='', $fecha_inicial_listado=''){
		/*if(!empty($qtype)&&!empty($qcampo)){
			$this->db->like($qtype,$qcampo);
			//var_dump($this->db);
		  }
		...
		return count($this->get_all_coordinador('','','','',$qtype,$qcampo,$where,$campo_where='',$arr_prog,$arr_nivel, $qtype2, $qcampo2, $fecha_inicial_listado));
	}
	public function get_all($page='',$total='',$order='',$campo_order='',$qtype='',$qcampo='',$where='',$campo_where='',$qtype2='',$qcampo2='', $imprimir=false, $fecha_inicial_listado=''){		
...
//if($qcampo!='')echo $this->db->last_query();
	...
	public function get_all_coordinador($page='',$total='',$order='',$campo_order='',$qtype='',$qcampo='',$where=array(),$campo_where='',$arr_prog,$arr_nivel,$qtype2='',$qcampo2='', $fecha_inicial_listado='',$arr_materias=null){
...
//if($qcampo!='')echo $query;
	...
	public function validarItemCodigoCrn($codigo,$crn,$periodo){		
		$query = $this->db->get_where($this->tableName(), array('sol_uidnumber' => $codigo,'sol_ins_crn' => $crn,'sol_periodo' => $periodo));
		$resultado = $query->result_array();
			//$resultado =$query->num_rows(); 		
                return $resultado;
	}	
	public function validarCrnBloqueado($crn){				
		$query = $this->db->get_where("ch_crns_bloqueados", array('crn' => $crn));
		$resultado = $query->result_array();	
	...
	public function reporte_solicitudes($conlimit,$programa='',$estado=0,$fec_ini='',$fec_fin='',$inicio=0,$cantidad=20,$order='',$ordertype='ASC',$arr_prog,$arr_nivel){
        ...
        public function reporte_solicitud_basico($conlimit,$programa='',$estado=0,$fec_ini='',$fec_fin='',$inicio=0,$cantidad=20,$order='',$ordertype='ASC',$arr_prog,$arr_nivel){
	...
	public function count_reporte_solicitud_basico($programa='',$estado=0,$fec_ini='',$fec_fin='',$inicio=0,$cantidad=20,$order='',$ordertype='ASC',$arr_prog,$arr_nivel){		
		return count($this->reporte_solicitud_basico(false,$programa,$estado,$fec_ini,$fec_fin,$inicio,$cantidad,$order,$ordertype,$arr_prog,$arr_nivel));
	}
